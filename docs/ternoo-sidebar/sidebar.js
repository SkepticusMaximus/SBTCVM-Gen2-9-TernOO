// Item types:
//   text   - bundled .txt file, rendered in <pre>
//   iframe - bundled .html file, rendered in iframe
//   link   - external URL, opened in new tab
...
const items = [
  ...
  },
  {
    ...
    tag: 'tutorial',
    desc: 'Binary, ternary, decimal, hex compared. Use with Object Inspector.',
    companion: 'inspector'
  },
...
function renderList(filter) {
  ...
  if (totalCount === 0) {
    welcome.textContent = 'No items match filter';
...
function openItem(item) {
  ...
  // If this item has a companion tool, load the tool in the sidebar
  // and open the main content in a new browser tab.
  if (item.companion === 'inspector' && item.type === 'text') {
    // Open the text content in a new browser tab as a plain-text view
    window.open(item.file, '_blank');

    // Load the inspector into the sidebar viewer
    document.getElementById('contentHeader').textContent = 'TernOO Object Inspector (companion)';
    var compBody = document.getElementById('contentBody');
    compBody.innerHTML = '';
    var compFrame = document.createElement('iframe');
    compFrame.src = 'content/inspector.html';
    compBody.appendChild(compFrame);
...
function backToMenu() {
  ...
  document.getElementById('contentView').classList.remove('show');
